    ...
    def process_trip_updates(
        ...
        for f in bar:
            ...
            try:
                ...
                con.register("_batch", bdf)
                if not table_initialized:
                    con.execute("CREATE TABLE results AS SELECT * FROM _batch")
                    table_initialized = True
                else:
                    con.execute("INSERT INTO results SELECT * FROM _batch WHERE key_hash NOT IN (SELECT key_hash FROM results)")
        ...
        result_df = con.execute("SELECT * FROM results").pl().drop("key_hash")
        stats["benchmark_predictions"] = result_df.height
