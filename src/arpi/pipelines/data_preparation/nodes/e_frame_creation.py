...
def _read_trip_updates_for_file(
    zin: zipfile.ZipFile,
    tu_file: str,
) -> dict[str, list[gtfs_realtime_pb2.TripUpdate.StopTimeUpdate]]:
    feed = gtfs_realtime_pb2.FeedMessage()

    with zin.open(tu_file, "r") as f:
        feed.ParseFromString(f.read())

    updates_by_trip_id: dict[
        str,
        list[gtfs_realtime_pb2.TripUpdate.StopTimeUpdate],
    ] = {}
...
def _load_gtfs_trips(
    ...
    trip_files = [
        ...
        if name.startswith(f"{GTFS_ROOT}/") and name.endswith("/trips.txt")
    ]
...
def _load_gtfs_stops(
    ...
    stop_files = [
        name
        for name in zin.namelist()
        if name.startswith(f"{GTFS_ROOT}/") and name.endswith("/stops.txt")
    ]
...
def _load_gtfs_stop_times(
    ...
    stop_time_files = [
        name
        for name in zin.namelist()
        if name.startswith(f"{GTFS_ROOT}/") and name.endswith("/stop_times.txt")
    ]

    for stop_times_path in tqdm(
        stop_time_files,
        desc="Loading GTFS stop_times",
        unit="file",
    ):
        ...
        for row in stop_times.select(
            ...
            by_trip_sequence[(agency, trip_id, stop_sequence)] = item
            by_trip_stop_id.setdefault((agency, trip_id, stop_id), []).append(item)

    logger.info(
        "Loaded %d GTFS stop_time sequence reference(s)",
        len(by_trip_sequence),
    )

    return by_trip_sequence, by_trip_stop_id


def _load_agency_timezones(
    zin: zipfile.ZipFile,
) -> dict[str, ZoneInfo]:
    ...
    agency_files = [
        name
        for name in zin.namelist()
        if name.startswith(f"{GTFS_ROOT}/") and name.endswith("/agency.txt")
    ]
...
def write_vehicle_position_timesteps(
    input_zip_path: str,
    matches_zip_path: str,
    output_zip_path: str,
    force: bool = False,
) -> str:
    ...
    logger.info(
        "Loaded %d matched vehicle_positions/trip_updates pair(s)",
        len(vehicle_position_files),
    )

    previous_by_key: dict[
        tuple[str, str, str],
        tuple[str, gtfs_realtime_pb2.VehiclePosition],
    ] = {}
        ...
        with zipfile.ZipFile(
            ...
            with zout.open(OUTPUT_CSV_NAME, "w") as raw:
                ...
                for current_file in tqdm(
                    ...
                    desc="Writing vehicle_position timesteps",
                    unit="file",
                ):
                    ...
                    agency, service_date = parsed_path
                    ...
                    try:
                        ...
                        trip_updates_by_trip_id = _read_trip_updates_for_file(
                            zin,
                            tu_file,
                        )
                    except Exception:
                        logger.exception(
                            "Failed to read GTFS-RT protobuf pair: vp=%s tu=%s",
                            current_file,
                            tu_file,
                        )
                        skipped_files += 1
                    ...
                    for (trip_id, vehicle_id), current_vp in current_by_key.items():
                        ...
                        for stu in stop_time_updates:
                            ...
                            stop = (
                                stops_by_key.get((agency, resolved_stop_id), {})
                                if resolved_stop_id
                                else {}
                            )
                            ...
                            scheduled_arrival_posix = _gtfs_time_to_posix(
                                service_date,
                                scheduled_arrival,
                                agency_tz,
                            )
                            scheduled_departure_posix = _gtfs_time_to_posix(
                                service_date,
                                scheduled_departure,
                                agency_tz,
                            )

                            predicted_arrival = _stu_arrival_to_posix(
                                stu,
                                scheduled_arrival_posix,
                            )
                            predicted_departure = _stu_departure_to_posix(
                                stu,
                                scheduled_departure_posix,
                            )
