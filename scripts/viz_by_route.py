...
def _clear_all_caches() -> None:
    ...
    _load_stop_names.clear()
    _load_route_shapes.clear()
...
def _route_filter_expr(route_id: str) -> pl.Expr:
    return pl.col("route_id").cast(pl.String) == route_id


def _apply_route_filter(df: pl.DataFrame, route_id: str) -> pl.DataFrame:
    if route_id == "Toutes" or "route_id" not in df.columns:
        return df
    return df.filter(_route_filter_expr(route_id))


@st.cache_data
def _load_trip_ids_for_route(output_dir_str: str, agency: str, route_id: str) -> list[str]:
    gtfs_zip = Path(output_dir_str) / agency / "GTFS.zip"
...
def _route_filtered_scan(
    ...
    if "trip_id" not in schema_cols:
        return scan

    trip_ids = _load_trip_ids_for_route(str(output_dir), agency, route_id)
...
def _empty_chart_data() -> dict[str, pl.DataFrame]:
    return {
        "summary": pl.DataFrame({"total": [0], "accurate": [0], "mean_abs_err": [0.0]}),
        "bucket_stats": pl.DataFrame({"time_bucket": [], "n": [], "accuracy": []}),
        "route_overall": pl.DataFrame({"route_id": [], "n": [], "accuracy": []}),
        "route_bucket": pl.DataFrame({"route_id": [], "time_bucket": [], "accuracy": []}),
        "stop_overall": pl.DataFrame({"stop_id": [], "n": [], "accuracy": []}),
        "stop_bucket": pl.DataFrame({"stop_id": [], "time_bucket": [], "accuracy": []}),
        "hourly_overall": pl.DataFrame({"hour": [], "n": [], "accuracy": []}),
        "hourly_bucket": pl.DataFrame({"hour": [], "time_bucket": [], "n": [], "accuracy": []}),
        "error_sample": pl.DataFrame(),
    }


def _compute_chart_data(df: pl.DataFrame) -> dict[str, pl.DataFrame]:
    if df.is_empty():
...
def _load_chart_data(path_str: str) -> dict[str, pl.DataFrame]:
    ...
    cached = {n: _read_cache(cd, n) for n in frame_names}
    if all(v is not None for v in cached.values()):
        return cached  # type: ignore[return-value]

    frames = _compute_chart_data(pl.read_parquet(path))
...
def _build_benchmark_chart(error_sample: pl.DataFrame, bucket_stats: pl.DataFrame) -> go.Figure:
    ...
    colors = {"Précis": "#3ecfad", "Attente excessive": "#f5c518", "Manqué": "#e84b56"}
...
with st.sidebar:
    ...
    selected_date = st.date_input("Date", value=dates_dt[-1], min_value=dates_dt[0], max_value=dates_dt[-1])
...
with st.sidebar:
    st.divider()
    st.header("Filtres")
    selected_route_id = st.selectbox("route_id", ["Toutes"] + route_options)

if selected_route_id == "Toutes":
    ...
    filtered_data = _compute_chart_data(pl.read_parquet(path).filter(_route_filter_expr(selected_route_id)))
    summary = filtered_data["summary"].row(0, named=True)
    bucket_stats = filtered_data["bucket_stats"]
    route_overall = filtered_data["route_overall"]
    route_bucket = filtered_data["route_bucket"]
    stop_overall = filtered_data["stop_overall"]
    stop_bucket = filtered_data["stop_bucket"]
    hourly_overall = filtered_data["hourly_overall"]
    hourly_bucket = filtered_data["hourly_bucket"]
    error_sample = filtered_data["error_sample"]
...
with tab_summary:
    ...
    accuracy_value = summary["accurate"] / summary["total"] * 100 if summary["total"] else 0
    c2.metric("Précision globale", f"{accuracy_value:.1f}%")
...
with tab_routes:
    if route_overall.is_empty():
        ...
        gtfs_zip = output_dir / agency / "GTFS.zip"
        if gtfs_zip.exists():
            route_shapes = _apply_route_filter(_load_route_shapes(str(gtfs_zip)), selected_route_id)
            ...
            if not map_route_pl.is_empty():
                ...
                if not all_lats.is_empty() and not all_lons.is_empty():
    ...
    if stop_overall.is_empty():
        ...
        if gtfs_zip.exists():
            ...
            if not map_df.empty:
                ...
                st_folium(fmap, width="100%", height=520, returned_objects=[], key=f"stops_map_{layer_choice}_{selected_route_id}")
                st.divider()
...
with tab_predictions:
    ...
    if not tu_path.exists():
        ...
        pred_detail = _apply_route_filter(pred_detail, selected_route_id)
        ...
        event = st.dataframe(
            ...
            hide_index=True,
            width="stretch",
            key=f"pred_table_{pred_page}_{pred_page_size}_{selected_route_id}",
            column_config={
            ...
            if not has_vp and not has_stop:
                ...
                if has_stop:
                    folium.CircleMarker(
                        ...
                        fill_opacity=0.85,
                        weight=2,
                        tooltip=f"Arrêt : {row.get('stop_name') or row.get('stop_id')}",
                    ).add_to(fmap)
                ...
                st_folium(
                    ...
                    height=420,
                    returned_objects=[],
                    key=f"pred_map_{pred_page}_{pred_page_size}_{selected_route_id}_{selected_rows[0]}",
                )
...
with tab_vp_raw:
    ...
    if not vp_path.exists():
        ...
        vp_scan = _route_filtered_scan(vp_path, selected_route_id, output_dir, agency)
        total_vp_rows = vp_scan.select(pl.len()).collect().item()

        if total_vp_rows == 0:
            st.info("Aucune ligne Vehicle Positions pour ce filtre.")
        else:
            st.download_button(
                "Télécharger Vehicle Positions (CSV)",
                data=vp_scan.collect().write_csv().encode("utf-8"),
...
with tab_tu_raw:
    ...
    if not tu_path.exists():
        ...
        tu_scan = _route_filtered_scan(vp_path, selected_route_id, output_dir, agency)
        total_tu_rows = tu_scan.select(pl.len()).collect().item()

        if total_tu_rows == 0:
            st.info("Aucune ligne Trip Updates pour ce filtre.")
        else:
            st.download_button(
                "Télécharger Trip Updates (CSV)",
                data=tu_scan.collect().write_csv().encode("utf-8"),
